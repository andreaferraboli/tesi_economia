...
dalla teoria economica. I mercati privati non coprono il rischio demografico
aggregato, che è sistemico e per definizione non diversificabile, e inoltre sono affetti da asimmetrie informative che gravano sul mercato delle rendite
vitalizie e da una miopia intertemporale che, senza obbligo contributivo, lascia il
risparmio individuale al di sotto del livello necessario
...
Sul piano teorico il confronto tra i due modelli resta aperto; sul piano attuativo,
è il doppio onere di transizione a renderlo per l'Italia una strada difficilmente
percorribile nel breve e medio periodo \cite{Artoni2014}.
...
previdenziale ha attraversato decenni di interventi sul metodo di calcolo
(riforma Dini, 1995), sull'età di accesso (riforma Fornero, 2011) e sulle
finestre di uscita anticipata concesse a pressioni di breve periodo (Quota~100
nel 2019, Quota~103 nel 2023). Ciascuno ha risposto a una domanda contingente
senza tornare al problema fondativo: garantire a tutti, e non soltanto ai
lavoratori già prossimi alla quiescenza, una vecchiaia economicamente
sostenibile.
...
strutturalmente dopo il picco del 17,1\% previsto intorno al 2042-2043, quando
l'ultima coorte del \emph{baby boom} raggiungerà l'età pensionabile, per poi
scendere al 14,0\% nel 2070 grazie
all'applicazione generalizzata del metodo contributivo e alla stabilizzazione del
rapporto demografico tra pensionati e occupati \cite{MEF_RGS2025_NdA}. La parola
fragile non è qui un giudizio ma una proprietà misurabile: la discesa attesa del
rapporto spesa/PIL dopo il 2042 poggia su un recupero della produttività che lo
scenario di base della Ragioneria fissa all'1,04\%
medio annuo, contro lo 0,12\% del decennio 2015-2025,\footnote{L'1,04\% non è un
parametro che la Ragioneria sceglie in modo discrezionale: discende dalle ipotesi
macroeconomiche armonizzate dell'Ageing Working Group del Comitato di politica
economica dell'Unione europea, che la RGS adotta e che assumono una convergenza di
lungo periodo della produttività del lavoro verso il suo tasso di equilibrio,
trainata dall'accumulazione di capitale per addetto e dal recupero della
produttività totale dei fattori \cite{MEF_RGS2025}. La fragilità non sta dunque
nell'arbitrio del numero ma nella sua distanza dal dato recente: il decennio
2015-2025 include la coda della doppia recessione e lo shock pandemico, ed è
perciò un benchmark depresso, mentre l'1,04\% resta un'ipotesi tecnicamente
difendibile e al tempo stesso non garantita.} e il blocco del solo
adeguamento automatico dei requisiti basterebbe ad aggiungere circa 36 punti di
debito/PIL al 2070 \cite{OCPI2026, MEF_RGS2025_NdA}.
...
P = \frac{M_C^{NDC}}{e_L}
\qquad \text{con} \qquad
M_C^{NDC} = \sum_{j=1}^{L} C_j \,(1+g)^{L-j}
\label{eq:ndc}
\end{equation}

\noindent dove $M_C^{NDC}$ è il montante contributivo nozionale,
...
\paragraph{Un esempio illustrativo: il divario fra lordo e netto.}
L'esercizio che segue serve a isolare una sola cosa, la distanza fra tasso di
sostituzione lordo e netto, e va letto in questa chiave: i parametri sono scelti
per chiarezza espositiva, non per riprodurre il livello effettivo della pensione
contributiva. Si consideri un lavoratore dipendente con 35 anni di contributi,
retribuzione lorda costante di 30.000 euro e aliquota $c = 33\%$ (contributi
annui: 9.900 euro). Il tasso nozionale è posto all'$1{,}5\%$: è un valore
prudenziale e volutamente tondo, scelto perché qui conta solo il divario fra lordo
e netto e non il livello della pensione. Non coincide con la capitalizzazione
effettiva del montante, che nel metodo NDC segue la crescita del PIL nominale,
attorno al 2,7\% medio nelle proiezioni RGS (0,7\% reale più un deflatore del 2\%)
\cite{MEF_RGS2025_NdA}: usare quel tasso più alto gonfierebbe il montante e
renderebbe l'esempio meno trasparente, mentre un tasso reale modesto come l'1,5\%
costruisce un montante plausibile senza sovrastimarlo. Il livello di pensione che
ne risulta non va perciò letto come previsione, ma solo come termine di paragone
fra il prelievo che grava sul lavoratore e quello che grava sul pensionato; la
calibrazione realistica dei tassi di sostituzione è ripresa nel Capitolo~4.
\[
\begin{aligned}
M_C^{NDC}
...
contributi a proprio carico pari al 9,19\% della retribuzione lorda (circa
2.757 euro; \citealp{INPSAliquote2023}), che riducono l'imponibile IRPEF; la sua aliquota media effettiva
...
con le addizionali regionali e comunali (media nazionale 1,5\%, circa 380 euro\footnote{Valore illustrativo e prudenziale: l'addizionale regionale varia tra l'1,23\% e il 3,33\% a seconda della regione e quella comunale arriva fino allo 0,8\%; si assume qui un'aliquota combinata dell'1,5\% sull'imponibile, che tende a sottostimare il prelievo e quindi a non gonfiare il divario di adeguatezza. Fonte: Ministero dell'Economia e delle Finanze, Dipartimento delle Finanze \cite{MEF_DF_Addizionali}.}), l'imposta totale è di 5.282 euro. L'aliquota media effettiva si attesta quindi al 20,9\%, per una pensione netta di:
...
L'esempio è condotto a legislazione vigente, con scaglioni e detrazioni IRPEF 2025,
e in termini reali correnti. Non incorpora il \emph{fiscal drag} né l'indicizzazione
futura degli scaglioni. Soprattutto, il tasso di sostituzione che ne risulta,
superiore al 90\%, non è il livello che il metodo contributivo restituisce davvero:
è sovrastimato dall'ipotesi di retribuzione costante. Capitalizzare al tasso nozionale
contributi calcolati su un salario fermo a 30.000 euro gonfia il rapporto fra montante
e ultima retribuzione, e avvicina il risultato a quello del vecchio metodo retributivo.
In un profilo realistico, con la retribuzione che cresce lungo la carriera in linea
con la base di capitalizzazione, lo stesso calcolo restituisce un tasso di sostituzione
dell'ordine del prodotto fra aliquota, anni di versamento e coefficiente, attorno
ai due terzi dell'ultima retribuzione. L'esempio va quindi preso per ciò che misura,
il cuneo fra lordo e netto a parità di importo, non per il livello della pensione
futura, che le proiezioni della Ragioneria Generale dello Stato collocano molto più
in basso e che le righe seguenti riportano.

Il calcolo al netto precisa la dimensione del problema demografico senza attenuarlo.
Se il coefficiente di trasformazione scende al 5,2\% nel 2040 per effetto dell'aumento
...
\cite{MEF_RGS2025}. Il passaggio dal lordo al netto non cambia il quadro: il
vantaggio fiscale di cui gode il pensionato rispetto al lavoratore attivo vale
in misura simile per la coorte del 2010 e per quella del 2070, e non compensa
quindi la caduta strutturale del tasso di sostituzione prodotta dal metodo
contributivo. Lo stesso scarto si legge sui valori netti: il tasso di sostituzione netto del
dipendente privato scende dall'82,7\% del 2010 al 64,1\% del 2070
\cite{MEF_RGS2025, MEF_RGS2025_NdA}. La previdenza
...
\item[\textbf{Domanda 3 (strategie individuali).}]
Quali comportamenti e quali scelte economiche e finanziarie deve mettere in
campo chi entra oggi nel mercato del lavoro per costruirsi il miglior futuro
pensionistico possibile, in un sistema che ha progressivamente trasferito su di
lui il rischio demografico e il rischio di longevità? E queste scelte sono
ugualmente accessibili a chi ha carriere regolari e a chi le ha discontinue, o
la capacità stessa di agire è distribuita in modo diseguale e diventa un
ulteriore fattore di disuguaglianza?
\end{description}
...
per tutelarsi? Il quarto capitolo, attraverso esercizi illustrativi su diversi profili di
carriera della Generazione Z, esamina la \textbf{regressività implicita} del
metodo contributivo e l'accesso diseguale alla previdenza complementare, e
delinea un \textit{framework} di pianificazione
